...
abd_verify(abd_t *abd)
{
	...
	if (abd_is_linear(abd)) {
		...
		ASSERT3U(abd->abd_u.abd_scatter.abd_offset, <, PAGE_SIZE);
...
abd_alloc(size_t size, boolean_t is_metadata)
{
	...
	abd->abd_u.abd_scatter.abd_chunk_size = PAGE_SIZE;
...
abd_get_offset(abd_t *sabd, size_t off)
{
	...
	if (abd_is_linear(sabd)) {
		...
		abd->abd_u.abd_scatter.abd_offset = new_offset % PAGE_SIZE;
		abd->abd_u.abd_scatter.abd_chunk_size = PAGE_SIZE;

		/* Copy the scatterlist starting at the correct offset */
		(void) memcpy(&abd->abd_u.abd_scatter.abd_chunks,
		    &sabd->abd_u.abd_scatter.abd_chunks[new_offset / PAGE_SIZE],
...
abd_iter_map(struct abd_iter *aiter)
{
	...
	if (abd_is_linear(aiter->iter_abd)) {
		...
		aiter->iter_mapsize = PAGE_SIZE - offset;
		paddr = abd_map_chunk(
