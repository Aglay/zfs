...
static int
zfs_parse_option(char *option, int token, substring_t *args, vfs_t *vfsp)
{
	switch (token) {
	case TOKEN_RO:
 ...
 */
static int
zfs_parse_options(char *mntopts, vfs_t **vfsp)
{
	vfs_t *tmp_vfsp;
	...
	while ((p = strsep(&t, ",")) != NULL) {
		...
		error = zfs_parse_option(p, token, args, tmp_vfsp);
		if (error) {
			strfree(tmp_mntopts);
			strfree(tmp_vfsp->vfs_mntpoint);
			kmem_free(tmp_vfsp, sizeof (vfs_t));
			return (error);
		}
	}
...
zfsvfs_free(zfsvfs_t *zfsvfs)
{
	...
	if (zfsvfs->z_vfs) {
		strfree(zfsvfs->z_vfs->vfs_mntpoint);
		kmem_free(zfsvfs->z_vfs, sizeof (vfs_t));
	}
...
zfs_domount(struct super_block *sb, zfs_mnt_t *zm, int silent)
{
	...
	error = zfs_parse_options(zm->mnt_data, &zfsvfs->z_vfs);
	if (error)
		goto out;
...
{
	zfsvfs_t *zfsvfs = sb->s_fs_info;
	vfs_t *vfsp, *unreg_vfsp = zfsvfs->z_vfs;
	int error;

	error = zfs_parse_options(zm->mnt_data, &vfsp);
	if (error)
		return (error);
	...
	if (unreg_vfsp) {
		strfree(unreg_vfsp->vfs_mntpoint);
		kmem_free(unreg_vfsp, sizeof (vfs_t));
	}
